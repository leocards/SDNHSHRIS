...
const ConfirmApproval: React.FC<ConfirmApprovalProps> = ({
    ...
    const onConfirm = (approval: string) => {
        router.post(route('myapproval.locatorslip.approval', [id]), {
            verdict: approval
        }, {
    ...
    return (
        <Modal
            show={show}
            onClose={onClose}
            title={`${verdict} Locator Slip`}
            maxWidth="lg"
        >
            {approval ===  'approved' && <div className="text-center my-10">
                Please confirm your approval of the Locator Slip of <br /> {user.full_name}
            </div>}

            {approval ===  'disapproved' && <div className="text-center my-10">
                Please confirm your disapproval of the Locator Slip of <br /> {user.full_name}
            </div>}
